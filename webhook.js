 ...
 * 🚀 PRODUÇÃO
 */
app.post("/tasinsulp/webhook/prd", async (req, res) => {
    ...
    try {
        ...
        const response = await fetch(
            `https://api.mercadopago.com/v1/payments/${paymentId}`,
            {
                headers: {
                    Authorization:
                        `Bearer ${process.env.MP_TOKEN_PRD_BRU}`
                }
            }
        );
 ...
 * 🧪 SANDBOX
 */
app.post("/tasinsulp/webhook/hml", async (req, res) => {
   ...
   try {
        ...
        const response = await fetch(
            `https://api.mercadopago.com/v1/payments/${paymentId}`,
            {
                headers: {
                    Authorization:
                        `Bearer ${process.env.MP_TOKEN_HML_BRU}`
                }
            }
        );
